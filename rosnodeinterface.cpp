...
// Generated 17-Mar-2022 14:37:28
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"time_to_collision\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(time_to_collision_M, (NULL));
        time_to_collision_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(time_to_collision_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(time_to_collision_M)
                == (NULL)) || rtmGetStopRequested(time_to_collision_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(time_to_collision_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
